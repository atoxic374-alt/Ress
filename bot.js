...
client.on('guildMemberUpdate', async (oldMember, newMember) => {
    try {
        ...
        for (const [roleId, role] of removedRoles) {
            ...
            if (!newMember.user?.bot) {
                specialRoleChanges.push({ action: 'remove', roleId, role, roleEntry });
            }
        }

        if (specialRoleChanges.length && !newMember.user?.bot) {
            for (const change of specialRoleChanges) {
                const executor = await getRecentRoleUpdateExecutor(newMember.guild, newMember.id, change.roleId, change.action);
