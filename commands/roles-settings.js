    ...
    embeds: [
      colorManager.createEmbed()
        ...
        .setDescription(
          ...
          `*Now : ${original.length ? original.map(id => `<@&${id}>`).join(' ') : 'لا يوجد (إداري فقط)*'}`
        )
        .setTimestamp()
    ]
  });
  ...
  if (type === 'member') {
    payload.components = [
      ...
      new ActionRowBuilder().addComponents(
        ...
        new ButtonBuilder().setCustomId('customroles_member_action_info').setLabel('معلومات الرول').setEmoji('<:emoji_41:1478860880769847429>').setStyle(ButtonStyle.Secondary),
...
async function buildTopRolesEmbed(guild, guildConfig) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(ranked.slice(0, 10).map((role, index) => (
      ...
      ` <:emoji_87:1442988617294413864> <@${role.ownerId}>\n` +
      ` <:emoji_85:1442986444712054954> ${role.messages} رسالة\n` +
      ` <:emoji_85:1442986413510627530> ${formatDuration(role.voice)}**`
    )).join('\n\n') || '**لا توجد بيانات بعد.**')
    .setColor(colorManager.getColor ? colorManager.getColor() : '#2f3136')
    .setTimestamp()
...
async function handleCustomRolesInteraction(interaction, client, BOT_OWNERS) {
  ...
  if (interaction.customId === 'customroles_request_button') {
    ...
    const colorInput = new TextInputBuilder()
      ...
      .setLabel('لون الرول (اختياري - مثال : #Ffffff)')
      .setStyle(TextInputStyle.Short)
      .setRequired(false);
  ...
  if (interaction.customId === 'customroles_request_modal') {
    ...
    if (rawColor && rawColor.trim() && !parsedColor) {
      await interaction.editReply({ content: '**❌ صيغة اللون غير صحيحة. استخدم مثال** : ___#ff9900___' });
      return;
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'لون الرول', value: parsedColor || 'N/A' },
        { name: 'اقصى عدد', value: parsedMaxMembers ? `${parsedMaxMembers}` : 'N/A' },
        { name: 'رول العضو', value: hasAdminRole ? (member.roles.cache.find(role => adminRoles.includes(role.id)) ? `<@&${member.roles.cache.find(role => adminRoles.includes(role.id)).id}>` : 'إداري') : `<@&${requesterAllowedRole.id}>` }
      )
      .setColor(colorManager.getColor ? colorManager.getColor() : '#2f3136')
      .setThumbnail(interaction.client.user.displayAvatarURL({ size: 128 }));

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`customroles_request_approve_${interaction.user.id}`).setLabel('Create?').setEmoji('<:emoji_41:1430334120839479449>').setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId(`customroles_request_reject_${interaction.user.id}`).setLabel('Reject?').setEmoji('<:emoji_39:1430334088924893275>').setStyle(ButtonStyle.Danger)
    );
  ...
  if (interaction.customId.startsWith('customroles_reject_modal_')) {
    ...
    const member = await interaction.guild.members.fetch(userId).catch(() => null);
    if (member) {
      await member.send(`**❌ تم رفض طلب الرول الخاص. السبب :\m ${reason}**`).catch(() => {});
    }

    const updatedEmbed = interaction.message?.embeds?.[0]
      ? EmbedBuilder.from(interaction.message.embeds[0])
      : colorManager.createEmbed().setTitle('Rejected Prv Role ');
    updatedEmbed
      .setFields(
