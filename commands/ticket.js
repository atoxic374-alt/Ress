...
const name = 'ticket';
const aliases = ['تكت', 'tclose', 'اغلاق', 'قفل', 'اقفال', 'myticket', 'نقاطي', 'tadd', 'tremove', 'tchange', 'تغيير', 'ttop', 'نقاط', 'tname', 'اسم', 'remind', 'تنبيه', 'استدعاء', 'points', 'tm', 'treset', 'tmreset'];
...
async function execute(message, args, { BOT_OWNERS = [], ADMIN_ROLES = [] }) {
  ...
  if (invokedToken.endsWith('points')) {
    await handlePointsAdjustMessage(message, args);
    return;
  }
